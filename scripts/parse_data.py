...
def parse_data(dataset_id):
    ...
    '''
    json_file = '../../download_data/data/%s.json' % dataset_id
    ...
    if os.path.exists(out_dir):
        print('Parsing output directory already exists, exiting.')
        return
    os.makedirs(out_dir)
    ...
    for p in tqdm(participants.keys()):
        ...
        # create dictionary that corrects the words
        ...
        correction_dict = {}
        all_words = list(timeseries_df.adjective.unique()) + list(final_impress_df.adjective.unique())
        for word in all_words:
            correction_dict[word] = sym_spell.word_segmentation(word).corrected_string

        # correct and save timeseries
        timeseries_df['corrected'] = timeseries_df.adjective.map(correction_dict)
        timeseries_df.to_csv(os.path.join(out_dir, 'timeseries.csv'))

        # correct and save final impressions
        final_impress_df['corrected'] = final_impress_df.adjective.map(correction_dict)
